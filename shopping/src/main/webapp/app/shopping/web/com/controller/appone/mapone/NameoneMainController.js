Ext.define('Shopping.shopping.web.com.controller.appone.mapone.NameoneMainController', {
     extend: 'Shopping.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.NameoneMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#NameoneTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#NameoneForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#NameoneGrid');
               var tree = this.view.down('#NameoneTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#NameoneForm');
          var grid = this.view.down('#NameoneGrid');
          var tree = this.view.down('#NameoneTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#NameoneGrid');
               ...
               var formPanel = this.getView().up().down('#NameoneForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#NameoneTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#NameoneForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#NameoneGrid').store;
          gridStore.removeAll();
